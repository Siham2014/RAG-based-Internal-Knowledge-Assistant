...
SUMMARY_PATH = (
    OUTPUT_DIR
    / "confidence_gate_features_summary.json"
)

MANIFEST_PATH = (
    OUTPUT_DIR
    / "confidence_gate_features_manifest.json"
)
...
def load_existing_rows(
    ...
    Cela permet de reprendre automatiquement le benchmark
...
def main() -> None:
    ...
    # Les seuils n'ont aucun effet ici :
    # nous utilisons seulement extract_features().
    ...
    for index, record in enumerate(
        ...
        try:
            ...
            print(
                ...
                f"Cosine: "
                f"{features.top1_cosine_similarity} | "
                f"Total: "
                f"{response.timings.total_time_ms:.2f} ms"
            )

        except Exception as error:
            # Les lignes déjà terminées restent sauvegardées.
            write_rows(
                FEATURES_PATH,
                output_rows,
            )
    ...
    manifest = {
        "benchmark_name": (
            "confidence_gate_feature_collection"
        ),
